...
# write.csv(ref_pat, 'refeval_080819.csv', row.names = F)
  ...
  # inner_join(ascent %>%
  #              mutate_each(as.character, ccn) %>%
  #              select(ccn, study.group) %>%
  #              distinct(), 
  #            by='ccn') %>%
  # group_by(ccn) %>%
  # mutate(ref_fac = sum(total_referrals>0)/n()) %>%
  # ungroup %>%
  # select(ccn, ref_fac) %>%
  # distinct(ccn, .keep_all = T) %>%
  # mutate(referred=ifelse(is.na(total_referrals)|as.Date(referral_date)>as.Date(FIRST_SE, '%m/%d/%Y')+365.25, 0, 1))
  
  
t.test(ref_fac~study.group, data=ref_pat)


# Replicate Laura's plot
# test = pat1 %>%
#   left_join(rbind(ref, ref1), by='USRDS_ID') %>%
#   mutate(referred=ifelse(is.na(total_referrals)|as.Date(referral_date)>as.Date(FIRST_SE, '%m/%d/%Y')+365, 0, 1)) %>%
#   group_by(PROVUSRD) %>%
#   summarise(ref_fac = sum(referred)/n()) %>%
#   ungroup %>%
#   arrange(ref_fac) %>%
#   mutate(id=row_number())# 
# 
# ggplot(test, aes(x=id, y=ref_fac*100)) +
#   scale_y_continuous(breaks = seq(0, 100, 10), limits=c(0, 100))+
#   scale_x_continuous(breaks=NULL)+
#   labs(y='% of Dialysis Patients, 2012-2016', x='End-Stage Renal Disease Network 6 Dialysis Facilities (n=795)')+
#   ggtitle('Variation in Early Steps in the Kidney Transplant Process \n among ESRD Network 6 Dialysis Facilities, 2012-2016')+
#   geom_line(color='#ED7D31')+
#   geom_area(fill='#ED7D31')+
#   geom_hline(yintercept=39.5, size=1, linetype = 'dotted')+
#   geom_text(aes(label="Median % referred: \n 39.50%", x=700, y=35), color='Black', size=5)+
#   theme_bw()+
...
# check ASCENT sample size
# pat2 = pat %>%
#   filter(ADRIND ==1) %>%
#   left_join(rxhist60_1, by='USRDS_ID') %>%
#   filter(!is.na(PROVUSRD) &
#            as.Date(ENDDATE, '%m/%d/%Y')>as.Date('2016/09/30') &
#            RXGROUP %in% c("1", "2", "3", "5", "7", "9", "T") &
#            between(INC_AGE, 18, 80)) %>%
#   left_join(wlki_1, by='USRDS_ID') %>%
#   filter(!(as.Date(FIRST_SE, '%m/%d/%Y')>as.Date(EDATE,'%m/%d/%Y') & !is.na(EDATE))) %>%
#   mutate(ccn = sub('^[0]+','', PROVHCFA)) %>%
#   inner_join(ascent %>%
#                mutate_each(as.character, ccn) %>%
#                select(ccn, study.group) %>%
#                distinct(), 
...
# 
# pat2 %>%
#   arrange(as.Date(FIRST_SE, '%m/%d/%Y')) %>%
#   select(FIRST_SE) %>%
#   head(1)
# 
# ascent %>%
#   arrange(as.Date(survey.date, '%m/%d/%Y')) %>%
#   select(survey.date) %>%
#   head(3)
